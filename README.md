...
* Device extensions
  ...
  * VK_EXT_swapchain_maintenance1 (For Headless and Wayland only)
  * VK_KHR_swapchain_maintenance1 (For Headless and Wayland only)
...
The Vulkan WSI Layer has been validated against Vulkan header version 1.4.356.
...
| `WSI_EMULATE_DMABUF_EXPORT=0` | Don't emulate dma-buf export. Otherwise, for drivers that can import dma-bufs but not export them (the Mali driver), memory allocated for dma-buf export comes from a dma-buf heap and is imported, and imported dma-bufs can be exported again. |
